...
pub(crate) struct EnhanceResembleDenoiseArgs {
    ...
    /// Inference runtime executing the model. Both produce the same result,
    /// but not at the same speed: paired with `--device metal`, `burn` is
    /// the fastest combination for this network by more than twice. It
    /// needs a build with the `burn` feature enabled and computes in f32
    /// only.
    #[arg(
        long,
        value_enum,
