...
need the lost part, recognize those pages with `ocr`.
...
`kind` is `text`, `mixed`, `scanned` or `image`. A page's `reason` is `scanned`,
`no_text`, `vector_text` or `no_unique_text`. `pages_needing_ocr`, `issues`,
`fonts_repaired` and `out` appear only when there is something to report.
...
`parse_failed`, `encrypted` (pass `--password`), `no_text_layer` (use `ocr`),
`invalid_options` (a `--pages` value the document cannot honour), `io_error`.
