//! The text detector: `PP-OCRv5_mobile_det`, a PP-LCNetV3 backbone under an
//! RSE feature-pyramid neck and a differentiable-binarization head.
//!
//! The network takes a page whose sides are multiples of 32 and returns a
//! single-channel probability map **at the input resolution** — the head ends
//! with two stride-2 transposed convolutions that undo the pyramid's quarter
//! scale, and the sigmoid is inside the exported graph. Everything after that
//! (thresholding, contours, boxes) is geometry and lives in [`super::db`].
//!
