...
runtime by default (as do `text structify`, `text punctuate`, and both TTS
engines, `tts qwen3-tts` and `tts espeech`). An alternative [burn](https://github.com/tracel-ai/burn)
...
runs entirely locally. Two engines, and choosing between them is choosing how
the voice is decided:

- **`qwen3-tts`** — ten languages, nine **preset** voices to pick from.
- **`espeech`** — Russian only, and the voice is **cloned** from a recording you
...
| `--text-file`, `--text-format`, `--pause-ms`, `--levels`, `--audio-encoder`, `--bitrate` | — | The shared `tts` options above. |
...
`asr gigaam`, three seeds each: `На ф+орзаце была наклеена старая карта.` comes
...
Shortening them is a job for another trakktor command, which is what the third
row above measures:
...
**Long text** works exactly as above (paragraphs, `--pause-ms`, `--levels`), with
...
- **`tts espeech`** — [F5-TTS](https://github.com/SWivid/F5-TTS) (MIT) by
